...
def process_gencat(df_minors_base, df_outsiders_base, input_dir, input_csv=None):
    ...
    df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'] = \
        df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'].astype(str)
    df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'] = \
        df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'].apply(eliminar_corchetes)
    df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'] = \
        df_minors_combined['ContractFolderStatus.ProcurementProject.TypeCode'].apply(convert_to_object_array)
    #import pdb; pdb.set_trace()
...
def main():
    ...
    if not args.no_download and administration in ['zaragoza', 'madrid', 'gencat', 'all']:
        ...
        if administration == 'all':
            ...
            if administration == 'zaragoza':
                ...
                detail_base_url = 'https://www.zaragoza.es/sede/servicio/contratacion-publica'
                file_path = os.path.join(input_dir, 'contratos_zaragoza_detalles.json')
                ...
                download_contracts_madrid(start_url, input_dir, args.start_year)
            elif administration == 'gencat':
                domain = "analisi.transparenciacatalunya.cat"
                dataset_identifier = "ybgg-dgi6"
                download_contracts_gencat(domain, dataset_identifier, input_dir, "contratacion_publica_catalunya_completo.csv")   
    
    # Paso 2: Integrar los datos descargados
    if administration in ['zaragoza', 'all']:
        ...
        zaragoza_file = os.path.join(input_dir, 'contratos_zaragoza_detalles.json')
        if administration == 'all':
